...
//Get All Users
export const getAllOwnersApi = async (token) => {
  try {
...
//Get user by _id
export const getOwnerByIdApi = async ({cedula, token}) => {
  try {
    const req = await fetch(`${API_BASE_URI}/owner/${cedula}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
//Create Owner
export const registerOwnerAPI = async ({body, token}) => {
  ...
  //Update User
export const updateOwnerAPI = async (id, body, token) => {
  try {
    console.log(token)
